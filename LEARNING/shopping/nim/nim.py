...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        """
        best_q = ()
        # No available actions in `state`
        if not Nim.available_actions(state):
            return 0
    
        for action in Nim.available_actions(state):
            actual_q = self.get_q_value(state, action)
            if not best_q:
                best_q = (state, action)
            else:
                if actual_q > self.get_q_value(best_q[0], best_q[1]):
                    best_q = (state, action)
        return self.get_q_value(best_q[0], best_q[1])
    ...
    def choose_action(self, state, epsilon=True):
        ...
        # Get the best reward the state can get
        ...
        for action in available_actions:
            actual_q = self.get_q_value(state, action)
            if actual_q == max_reward:
                best_action = action
                break
        
        # In case no rewards are assigned to the state-action choose an action randomly
        if max_reward == 0:
